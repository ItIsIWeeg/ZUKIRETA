...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		{
			case 'stage': //Week 1
				var bg:BGSprite = new BGSprite('zuki-color', -200, 0, 0, 0);
				bg.scale.set(2, 2);
				add(bg);

				var newBG:FlxBackdrop = new FlxBackdrop(Paths.image('white_stuff'));
				newBG.velocity.set(20, 20);
				newBG.scrollFactor.set(0, 0);
				add(newBG);
	...
	function noteMissPress(direction:Int = 1, ?ghostMiss:Bool = false):Void //You pressed a key when there was no notes to press for this key
	{
		if (!boyfriend.stunned)
		{
			...
			boyfriend.setColorTransform(0.6, 0.5, 1, boyfriend.alpha, 0, 0, 0, 0);
			vocals.volume = 0;
		}
	}
		...
		if (!note.wasGoodHit)
		{
			...
			if(!note.noAnimation) {
				...
				if(note.noteType == 'GF Sing') {
					...
					if (!dad.danced)
					{
						boyfriend.animation.curAnim.curFrame = (dad.animation.curAnim.curFrame + 10);
					}
					else
					{
						boyfriend.animation.curAnim.curFrame = (dad.animation.curAnim.curFrame);
					}
