...
class ControlsSubstate extends MusicBeatSubstate {
	...
	public function new() {
		...
		for (i in 0...optionShit.length) {
			...
			var optionText:Alphabet = new Alphabet(0, (10 * i), optionShit[i], (!isCentered || isDefaultKey), false, 0.05, 0.75);
...
class PreferencesSubstate extends MusicBeatSubstate
{
	...
	public function new()
	{
		...
		showCharacter = new Character(500, 170, 'bf', true);
		...
		for (i in 0...options.length)
		{
			...
			var optionText:Alphabet = new Alphabet(0, 70 * i, options[i], false, false, 0.05, 0.75);
			optionText.isMenuItem = true;
			if(isCentered) {
				optionText.screenCenter(X);
				optionText.forceX = optionText.x;
		...
		descText = new FlxText(0, 600, FlxG.width, "", 32);
	...
	function changeSelection(change:Int = 0)
	{
		...
		switch(options[curSelected]) {
			case 'Framerate':
				daText = "Pretty self explanatory, isn't it? Default value is 60.";
			case 'Note Delay':
				daText = "Changes how late a note is spawned. Useful for preventing audio lag from wireless earphones.";
			case 'FPS Counter':
				daText = "If unchecked, hides FPS Counter.";
			case 'Low Quality':
				daText = "If checked, disables some background details, decreases loading times and improves performance.";
			case 'Persistent Cached Data':
				daText = "If checked, images loaded will stay in memory until the game is closed, this increases memory usage,\nbut basically makes reloading times instant.";
			case 'Anti-Aliasing':
				daText = "If unchecked, disables anti-aliasing, increases performance at the cost of the graphics not looking as smooth.";
			case 'Downscroll':
			...
			case 'Ghost Tapping':
				daText = "If checked, you won't get misses from pressing keys while there are no notes able to be hit.";
			case 'Swearing':
			...
			case 'Hide Song Length':
				daText = "If checked, the bar showing how much time is left will be hidden.";
		}
